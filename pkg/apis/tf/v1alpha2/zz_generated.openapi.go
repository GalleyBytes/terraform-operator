		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"requireApproval": {
						SchemaProps: spec.SchemaProps{
							Description: "RequireApproval will place a hold after completing a plan that prevents the workflow from continuing. However, the implementation of the hold takes place in the tf.sh script. (See https://github.com/GalleyBytes/terraform-operator-tasks/blob/master/tf.sh)\n\nDepending on the script that executes during the workflow, this field may be ignored if not implemented by the user properly.\n\nTo approve a workflow using the official galleybytes implementation, a file needs to be placed on the workflow's persistent-volume:\n\n- $TFO_GENERATION_PATH/_approved_<uuid-of-plan-pod> - to approve the workflow\n\n- $TFO_GENERATION_PATH/_canceled_<uuid-of-plan-pod> - to deny and cancel the workflow\n\nDeleting the plan that is holding will spawn a new plan and a new approval will be required.",
							Default:     false,
							Type:        []string{"boolean"},
							Format:      "",
						},
				...
				Required: []string{"terraformModule", "terraformVersion", "backend", "requireApproval"},
			},
		},
		Dependencies: []string{
